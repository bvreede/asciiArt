# `asciiart` developer documentation
...
# install asciiart as an editable package
...
The second is to use `tox`, which can be installed separately (e.g. with `pip install tox`), i.e. not necessarily inside the virtual environment you use for installing `asciiart`, but then builds the necessary virtual environments itself by simply running:
...
# recursively check import style for the asciiart module only
isort --recursive --check-only asciiart

# recursively check import style for the asciiart module only and show
# any proposed changes as a diff
isort --recursive --check-only --diff asciiart

# recursively fix import style for the asciiart module only
isort --recursive asciiart
```
...
Visit
[https://test.pypi.org/project/asciiart](https://test.pypi.org/project/asciiart)
and verify that your package was uploaded successfully. Keep the terminal open, we'll need it later.

In a new terminal, without an activated virtual environment or an env directory:

```shell
cd $(mktemp -d asciiart-test.XXXXXX)
...
--extra-index-url https://pypi.org/simple asciiart
```
